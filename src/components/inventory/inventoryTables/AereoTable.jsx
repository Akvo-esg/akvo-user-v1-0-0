...
export default function AereoTable(props) {
  ...
  const validateEditDB = () => {
    if (
      editdescricaoFrotaDB &&
      editCombustivelIdDB &&
      editPeriodoConsumoDB &&
      editConsumoAnualDB > 0
    ) {
      return true;
    } else {
      return false;
  ...
  const handleEditDB = async () => {
    ...
    if (isValid) {
      ...
      await axios
        .patch(`${baseUrl()}/api/editInventory/${token.company_id}`, data)
  ...
  const handleDeleteDB = async (code) => {
    ...
    await axios
      .post(`${baseUrl()}/api/editInventory/${token.company_id}`, data)
      .then((res) => {
        props.updateList();
      });
  };
  ...
  const renderInventoryTable = (list) => {
    ...
    for (let i = 0; i < list.length; i++) {
      if (
        list[i].fonteEmissao === "Transportes" &&
        list[i].tipoEmissao === "Transporte Aéreo" &&
      ...
      return (
        <div className="mt-5 fadeItem">
          <h6 className="h5_title">
            Últimos dados cadastrados em {states.unidName} /{" "}
                ...
                <thead>
                  <tr>
                        ...
                        <th>
                          <div className="input-group input-group-sm">
                            <input
                              ...
                              onChange={(e) =>
                                setSearchIdentificador(e.target.value)
                              }
                            />
                    ...
                    {showMoreInfo && (
                      <>
                        <th className="text-center akvo-text-escopo1">
                          Emissões t CO<sub>2</sub>
                          <br />
                          (Combustíveis fóssies)
                        </th>
                        <th className="text-center akvo-text-escopo1">
                          Emissões t CH<sub>4</sub>
                          <br />
                          (Combustíveis fóssies)
                        </th>
                        <th className="text-center akvo-text-escopo1">
                          Emissões t N<sub>2</sub>O<br />
                          (Combustíveis fóssies)
                        </th>
                        <th className="text-center akvo-text-escopo1">
                          Emissões t CO<sub>2</sub>
                  ...
                  {inventoryList
                    .filter((val) => {
                      if (searchIdentificador == "") {
                        ...
                        val.identificador
                          .toLowerCase()
                          .includes(searchIdentificador.toLocaleLowerCase())
                      ) {
                        // console.log(val)
                        return val;
                      }
                    ...
                    .filter((val) => {
                      if (searchDescricao == "") {
                        ...
                        val.descricaoFrota
                          .toLowerCase()
                          .includes(searchDescricao.toLocaleLowerCase())
                      ) {
                        return val;
                      }
                    })
                        ...
                        <>
                          {editCodeDB === elem.code ? (
                              ...
                              <tr className="fadeItem" key={`edit${index}`}>
                                ...
                                <td>
                                  <input
                                    className="form-control form-control-sm"
                                    type="text"
                                    value={editIdentificadorDB}
                                    onChange={(e) =>
                                      setEditIdentificadorDB(e.target.value)
                                    }
                                  />
                                </td>
                                <td scopo="row">
                                  <input
                                    className="form-control form-control-sm"
                                    type="text"
                                    value={editdescricaoFrotaDB}
                                    onChange={(e) =>
                                      setEditdescricaoFrotaDB(e.target.value)
                                    }
                                  />
                                </td>
                                <td scopo="row">
                                  ...
                                  >
                                    ...
                                    <option value="6,Litros,Querosene de Aviação">
                                      Querosene de Aviação
                                    </option>
                                    <option value="7,Litros,Gasolina de Aviação">
                                      Gasolina de Aviação
                                    </option>
                                  </select>
                                </td>
                                <td scopo="row">
                                  {editPeriodoConsumoDB
                                    .charAt(0)
                                    .toUpperCase() +
                                    editPeriodoConsumoDB.slice(1)}
                                </td>
                                <td>
                                  <div className="input-group input-group-sm">
                                    <input
                                      className="form-control form-control-sm"
                                      id="editConsumoItemDB"
                                      type="number"
                                      disabled
                                      value={editConsumoAnualDB}
                                      placeholder="0"
                                      onChange={(e) =>
                                        setEditConsumoAnualDB(e.target.value)
                                      }
                                    />
                                    <span className="input-group-text">
                                      {editUnidadeDB}
                                  ...
                                  >
                                    <div className="modal-content">
                                      <div className="modal-header">
                                        ...
                                        >
                                          Editar o comentario do registro{" "}
                                          {editCodeDB}{" "}
                                        </h5>
                                        <button
                                          type="button"
                                      ...
                                      <div className="modal-body">
                                        <textarea
                                          ...
                                          onChange={(e) =>
                                            setEditComentarioDB(e.target.value)
                                          }
                                        />
                                      </div>
                            ...
                            <tr className="fadeItem">
                              ...
                              <td>{elem.emissoesTotais.toFixed(2)}</td>
                              <td>{elem.emissoesBiogenicas.toFixed(2)}</td>
                              {showMoreInfo && (
                                <>
                                  <td>{elem.emissoesCO2_F.toFixed(4)}</td>
                                  <td>{elem.emissoesCH4_F.toFixed(4)}</td>
                                  <td>{elem.emissoesN2O_F.toFixed(4)}</td>
                                  <td>{elem.emissoesCO2_B.toFixed(4)}</td>
                                  <td>{elem.emissoesCH4_B.toFixed(4)}</td>
                                  <td>{elem.emissoesN2O_B.toFixed(4)}</td>
                                  <td>{elem.userName}</td>
                                  <td>
                                    {elem.dateUpdated
                                      ? ` ${new Date(
                                ...
                                {idCompare(
                                  ...
                                  >
                                    ...
                                    <span
                                      ...
                                      data-bs-toggle="modal"
                                      data-bs-toggle-tooltip="true"
                                      data-bs-target="#deleteModalBD"
                                      data-bs-placement="bottom"
                                      title="Excluir"
                                      onClick={() =>
                                        setDeleteElemCodeDB(elem.code)
                                      }
                                    >
                                  ...
                                  <div className="modal-content">
                                    ...
                                    <div className="modal-footer">
                                      ...
                                      <button
                                        ...
                                        data-bs-dismiss="modal"
                                        onClick={() =>
                                          handleDeleteDB(deleteElemCodeDB)
                                        }
                                      >
                                        Excluir
                                      </button>
  ...
  return <>{renderInventoryTable(inventory)}</>;
}
